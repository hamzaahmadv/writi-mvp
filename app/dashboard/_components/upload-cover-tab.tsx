...
/*
<ai_context>
Component for uploading custom images as page covers, matching Notion's UI design
</ai_context>
*/
...
import { useUploadCover } from "@/lib/hooks/use-upload-cover"
...
export default function UploadCoverTab({
  onCoverSelect,
  onClose
}: UploadCoverTabProps) {
  const {
  ...
  const handleUpload = async () => {
    ...
    const success = await uploadCover(selectedFile)
    if (success && uploadedCover) {
      onCoverSelect(uploadedCover)
      onClose()
    }
  }
        ...
        {previewUrl && (
          <div className="relative">
            <img
              src={previewUrl}
              alt="Cover preview"
              className="h-32 w-80 rounded-md border border-gray-200 object-cover shadow-sm"
            />
          </div>
        )}
